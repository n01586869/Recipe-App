...
const Vegetarian = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Vegetarian`) // get list of Vegetarian recipes. this returns array of objects with only meal id and thumbnail
  ...
  const VegetarianScreen = () => {
    return( 
      <>
        {recipes[0] ? // Loading screen while waiting for fetch
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Vegetarian Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Vegetarian Screen" component={VegetarianScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Vegetarian Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
