...
const Side = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Side`) // get list of Sides. this returns array of objects with only meal id and thumbnail
  ...
  const SideScreen = () => {
    return( 
      <>
        ...
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Side Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Side Screen" component={SideScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Side Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
