...
const Pasta = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Pasta`) // get list of Pasta recipes. this returns array of objects with only meal id and thumbnail
  ...
  const PastaScreen = () => {
    return( 
      <>
        ...
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Pasta Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Pasta Screen" component={PastaScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Pasta Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
