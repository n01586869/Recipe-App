...
const Goat = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Goat`) // get list of Goat recipes. this returns array of objects with only meal id and thumbnail
  ...
  const GoatScreen = () => {
    return( 
      <>
        ...
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Goat Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Goat Screen" component={GoatScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Goat Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
