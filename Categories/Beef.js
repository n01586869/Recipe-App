...
const Beef = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Beef`) // get list of Beef recipes. this returns array of objects with only meal id and thumbnail
  ...
  const BeefScreen = () => {
    return( 
      <>
        ...
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Beef Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Beef Screen" component={BeefScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Beef Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
