...
const Miscellaneous = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Miscellaneous`) // get list of Miscellaneous recipes. this returns array of objects with only meal id and thumbnail
  ...
  const MiscellaneousScreen = () => {
    return( 
      <>
        ...
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Miscellaneous Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Miscellaneous Screen" component={MiscellaneousScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Miscellaneous Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
