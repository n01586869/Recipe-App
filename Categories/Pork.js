...
const Pork = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Pork`) // get list of Pork recipes. this returns array of objects with only meal id and thumbnail
  ...
  const PorkScreen = () => {
    return( 
      <>
        ...
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Pork Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Pork Screen" component={PorkScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Pork Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
