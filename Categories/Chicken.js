...
const Chicken = ({ route, navigation }) => {
  ...
  useEffect(() => {
    fetch(`${API}/${API_KEY}/filter.php?c=Chicken`) // get list of Chicken recipes. this returns array of objects with only meal id and thumbnail
  ...
  const ChickenScreen = () => {
    return( 
      <>
        ...
        <RecipeCardList navigation={navigation} recipes={recipes} screen={"Chicken Recipes"}/>
        :
  ...
  return(
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="Chicken Screen" component={ChickenScreen}/>
      <Stack.Screen initialParams={{setCurrentScreen}} name="Chicken Recipes" component={Recipe}/>
    </Stack.Navigator>
  )
}
